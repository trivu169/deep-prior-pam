...
clearvars; %-except p0_recon p0_recon_full p0_recon_interp p0_recon_interp_full;
...
Nx = num_points - 2 * PML_size;    % number of grid points in the x direction
Ny = num_points - 2 * PML_size;    % number of grid points in the y direction
...
sensor_angle = pi/2;%2*pi;%3*pi/2;      % [rad]
sensor_pos = [0, 0];        % [m]
num_sensor_points = ceil(512*sensor_angle/(2*pi));%256;%70;
...
% inverse crime
Nx = round(num_points, -2);           % number of grid points in the x direction
Ny = round(num_points, -2);           % number of grid points in the y direction
dx = x/Nx;          % grid point spacing in the x direction [m]
dy = y/Ny;          % grid point spacing in the y direction [m]
...
title('Init Press and Sens Distr')
...
colormap(getColorMap);
title('Reconstructed Initial Pressure (Interp)');
ylabel('x-position [mm]');
xlabel('y-position [mm]');
axis image;
...
%% Save Images
save(['p0_recon_',num2str(round(sensor_angle*180/pi)),'deg.mat'], 'p0_recon');
save(['p0_recon_interp_',num2str(round(sensor_angle*180/pi)),'deg.mat'], 'p0_recon_interp');
imwrite(uint8(255 * mat2gray(p0_recon)), ['p0_recon_',num2str(round(sensor_angle*180/pi)),'deg.png'])
imwrite(uint8(255 * mat2gray(p0_recon_interp)), ['p0_recon_interp_',num2str(round(sensor_angle*180/pi)),'deg.png'])